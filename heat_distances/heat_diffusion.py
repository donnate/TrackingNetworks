# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 11:57:41 2017
...
def compute_evolution_heat_diff(node1,list_heat_df1,list_heat_df2,preprocess=False,type_distance="auc",normalize=True,mode_diff="agg",plot=False, savefig=False,**kwargs):
    
    
    #area1=[(delta2[i]-delta1[i])*m1[i]-delta2[i]*test1[i]+test2[i]*delta1[i] for i in range(n-1)]
    if preprocess:
        list_nodes=np.union1d(kwargs['index_nodes1'],kwargs['index_nodes2'])
        n=len(list_nodes)
        try:      
            ind_nd1=list(kwargs['index_nodes1']).index(node1)
            ind1=list((list_heat_df1[kwargs['mode']]).iloc[:,ind_nd1])
            test1=np.sort(ind1+[0]*(n-len(ind1)))
        except:
            ind1=[0]*(n-1)+[1.0]
        try:
            ind_nd2=list(kwargs['index_nodes1']).index(node1)
            ind2=np.argsort((list_heat_df2[kwargs['mode']]).iloc[:,ind_nd2])
            test2=np.sort(ind2+[0]*(n-len(ind2)))
        except:
            ind2=[0]*(n2-1)+[1.0]
    else:
        test1=ind1
