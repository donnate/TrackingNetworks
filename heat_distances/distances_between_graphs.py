# -*- coding: utf-8 -*-
"""
Created on Sun Apr 23 13:47:04 2017
...
def distances_heatprints(heat_print1,heat_print2,index_node1,index_node2,type_comp="auc",mode_diff="agg",plot=False,savefig=False,filefig="plots/change_graph.png"):
    ...
    #taus=[1, 10, 25, 50]
    list_nodes=np.union1d(index_node1,index_node2)
    n=len(np.union1d(index_node1,index_node2))
    
    M=len(heat_print1)
    D_12= pd.DataFrame(np.zeros((M,n)),columns=list_nodes)
    for m in range(M):
        for nod in list_nodes:
            try:
                ind_nod1=list(index_node1).index(nod)
                ind1=list((heat_print1[m]).iloc[:,ind_nod1])
                ind1+=[0]*(n-len(index_node1))
            except:
                ind1=[0]*(n-1)+[1.0]
            ind1=np.array(np.sort(ind1))
            try:
                ind_nod2=list(index_node2).index(nod)
                ind2=list((heat_print2[m]).iloc[:,ind_nod2])
                ind2+=[0]*(n-len(index_node2))
            except:
                ind2=[0]*(n-1)+[1.0]
            ind2=np.array(np.sort(ind2))
            if type_comp=="corr":
                D_12[nod][m]=1-1.0/(np.linalg.norm(ind1)*np.linalg.norm(ind2))*(ind1.dot(ind2))
            elif type_comp=="auc":
                D_12[nod][m]=compute_auc(ind1,ind2,normalize=False,mode_diff=mode_diff)
            elif type_comp=="emd":
         ### Required params:  
         ### P,Q - Two histograms of size H
         ### D - The HxH matrix of the ground distance between bins of P and Q
                ...
                D_12[nod][m]==emd(np.array(hist1.tolist()[0]),np.array(hist2.tolist()[0]),D)
            else:
                 print "comparison type not implemented"
                 return np.nan
    if plot==True:
...
def distances_heatprints_mode(heat_print1,heat_print2,index_node1,index_node2,m=2,type_comp="auc",mode_diff="agg",plot=False,savefig=False,filefig="plots/change_graph.png"):
    ...
    D_12= pd.DataFrame(np.zeros((1,n)),columns=list_nodes)
    ...
    for nod in list_nodes:
        ...
        ind2=np.array(np.sort(ind2))
        if type_comp=="corr":
            D_12[n]=1-1.0/(np.linalg.norm(ind1)*np.linalg.norm(ind2))*(ind1.dot(ind2))
        elif type_comp=="auc":
            D_12[n]=compute_auc(test1,test2,normalize=False,mode_diff=mode_diff)
        else:
            print "comparison type not implemented"
            return 0
    if plot==True:
...
def chi_distance_aligned_graph(nodes_names1,nodes_names2,chi1,chi2,nb_J=30,range_t=range(0,150,3)):
    ...
    for i in range(len(intersect)):
        ...
        d+=1.0/Z*(np.linalg.norm(chi1[ind1,:]-chi2[ind2,:]))
        count+=1
    for i in range(len(diff1)):
        ind1=nodes_names1.index(diff1[i])
        d+=1.0/Z*(np.linalg.norm(chi1[ind1,:]-(1.0/len(nodes_names1)+circle)))
        count+=1
    for i in range(len(diff2)):
        ind2=nodes_names2.index(diff2[i])
        d+=1.0/Z*(np.linalg.norm(chi2[ind2,:]-(1.0/len(nodes_names2)+circle)))
        count+=1
    return d*1.0/count
