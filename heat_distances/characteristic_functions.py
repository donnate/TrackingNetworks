# -*- coding: utf-8 -*-
"""
Created on Thu May 11 15:01:07 2017
...
### Use characteristic function as input for doing PCA for instance:
import numpy as np
...
def featurize_characteristic_function(heat_print,t=[],nodes=[]):
    ...
    for tau in range(len(heat_print)):
        if (type(heat_print)==list) or ((type(heat_print)==dict and np.sum([heat_print.keys()[i]==i for i in range(len(heat_print))])==len(heat_print))):
            sig=heat_print[tau]
        else:
            key=heat_print.keys()[tau]
            sig=heat_print[key]
