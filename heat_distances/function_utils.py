# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 16:04:27 2017
...
def variance_without_diagonal(mat,recompute_mean=False):
    ...
    for i in range(mat.shape[1]):
        if recompute_mean:
            ...
            mu[i]=1.0/N
        var[i]=np.mean([(mat[j,i]-mu[i])**2 for j in range(mat.shape[0]) if j!=i])
    return var,mu
...
def entropy_naive(mat,centered=False,offset=False,norm=False):
    ...
    for i in range(mat.shape[1]):
        if  offset:
            ent[i]=np.mean([-mat2[j,i]*np.log(mat2[j,i]) for j in range(mat.shape[0]) if j!=i])
        else:
