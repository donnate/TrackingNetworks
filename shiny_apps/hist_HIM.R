...
hist_HIM <- function() {
  ...
  shinyApp(
    ...
    ui = fluidPage(responsive = FALSE,
                   # Application title
                   titlePanel("Histograms for the HIM Distance"),
                   ...
                   sidebarLayout(
                     sidebarPanel(
                       ...
                       sliderInput("y_max", label = "Histogram Height adjustment:",
                        ...
                        selectInput("B", label = "Bootstrap samples:",
                                   choices = c(50, 100, 200), selected = 50)
    ...
    server = function(input, output, session) {
      ...
      test_bootstrap1<-reactive({
        ...
        N=30
        B=input$B
        test_bootstrap_shiny_HIM(N,input$alpha,input$order_max,args,input$opts,B)
      })
      
      
      
      output$distPlot <- renderPlot({
        hist(test_bootstrap1()[1,],xlim=c(0,1),ylim=c(0,input$y_max),breaks=15,xlab="Distance",main="Histogram of the distance distribution", col = 'darkgray', border = 'white')
        par(new=TRUE)
        hist(test_bootstrap1()[2,],xlim=c(0,1),ylim=c(0,input$y_max),breaks=15,xlab="",ylab="",main="", col = 'blue', border = 'white')
        par(new=TRUE)
        hist(test_bootstrap1()[3,],xlim=c(0,1),ylim=c(0,input$y_max),breaks=15,xlab="",ylab="",main="", col = 'pink', border = 'white')
      })
